...
<div class="right" id="li07">
	<div class="right01">
		<img src="/MI-4-20/Public/Sun/img/04.gif" /> 订单管理 &gt; 查看订单 &gt; <!--<span>添加用户</span>-->
	</div>
	<div class="rightmain">
		<h2 class="text-center">订单管理</h2>
		<form style="width:40%;margin:0 auto" action="<?php echo U('index');?>" method="get">
			<div class="col-xs-4">
				<input class="form-control" value="<?php echo ($id); ?>" placeholder="请输入订单号" type="text" name="id">
			</div>
			<div class="col-xs-4">
				<input class="form-control" value="<?php echo ($name); ?>" placeholder="请输入用户名" type="text" name="keyword">
			</div>
		...
		<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
				...
				<td>姓名:<?php echo ($vo["oname"]); ?>&nbsp;&nbsp;地址:<?php echo ($vo["address"]); ?>&nbsp;&nbsp;联系方式:<?php echo ($vo["phone"]); ?></td>
				<td><?php echo ($vo["total"]); ?></td>
				<?php if($vo["ostatus"] == 1): ?><td class="text-primary">待付款</td>
				<?php elseif($vo["ostatus"] == 2): ?>
					<td class="text-danger">待发货</td>
				<?php elseif($vo["ostatus"] == 3): ?>
					<td class="text-info">待收货</td>
				<?php elseif($vo["ostatus"] == 4): ?>
					<td class="text-warning">已完成</td><?php endif; ?>
				
				<td style="width:300px">
				<?php if($vo["ostatus"] == 2): ?><a class="delivery" href="javascript:;" data-value='<?php echo ($vo["oid"]); ?>'>发货</a><?php endif; ?>
					<a class="orderon" href="javascript:;" data-value="<?php echo ($vo["oid"]); ?>" data-toggle="modal" data-target=".bs-example-modal-lg">订单详情</a>
				</td>
  ...
  <div class="modal-dialog modal-lg" role="document">
	...
	<script>
				...
				$('table .delivery').click(function(){
					...
					//ajax的请求php
					$.get("<?php echo U('Order/delivery');?>",{id:currenId},function(data){
				...
				$('table .orderon').click(function(){
					...
					$.get(
							"<?php echo U('Orderdetail/Orderdetail');?>",
							{id:currenId},
							function(data){
								// console.log(data);
								...
								for(var i = 0; i < data.length; i++) {
									...
									str += "<tr><td><img width='50px' src='.//MI-4-20/Public/imgs/"+data[i]['id']+'/'+data[i]['cover']+"'></td><td>"+data[i]['name']+" <span>"+data[i]['colorname']+"</span><span>"+data[i]['vname']+"</span></td><td>"+data[i]['goodsnumber']+"</td><td>"+data[i]['price']+"</td><td>"+data[i]['num']+"</td><td>"+data[i]['total']+"</td></tr>";
