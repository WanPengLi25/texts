...
<div class="right" id="li07">
	<div class="right01">
		<img src="/MI-4-20/Public/Sun/img/04.gif" /> 友情链接管理 &gt; 查看链接 <!--<span>订单详情</span>-->
	</div>
	<!-- 查看友情链接开始 -->
	<div class="rightmain">
		...
		<button type="button" class="btn btn-success btn-primary center-block" data-toggle="modal" data-target=".bs-example-modal-lg">添 　加</button>
			...
			<?php if(is_array($showa)): foreach($showa as $key=>$sh): ?><tr style="text-align:center">
					<td><?php echo ($sh["id"]); ?></td>
					<td class="text-center">
						<input type="text" name="keyword" disabled value="<?php echo ($sh["name"]); ?>" class="form-control jia center-block" style="width:200px;">
					</td>
					<td>
						<input type="url" name="url" disabled value="<?php echo ($sh["url"]); ?>" class="form-control you center-block" style="width:300px;">	
					</td>
					<td>
						<a class="btn btn-warning" href="javascript:;" data-value="<?php echo ($sh["id"]); ?>" >更改</a>
						<a class="btn btn-danger" href="javascript:;" data-value="<?php echo ($sh["id"]); ?>" >移除</a>
					</td>
     ...
     <h2 class="text-center">添加友情链接</h2>
			<table class="table table-hover">
				...
				<tr>
					...
					<form action="<?php echo U('Friendlinks/addFriendlinks');?>" method="get">
						<td class="text-center">
...
<!-- 添加友情链接结束 -->
	...
	<script>
		...
		$('body').on('click', '.btn-success', function(){
			...
			//请求ajax
			$.get("<?php echo U('Friendlinks/addFriendlinks');?>", function(data){
		...
		$('table').on('click', '.btn-danger', function(){
			...
			// 请求ajax
			$.get("<?php echo U('Friendlinks/delelink');?>", {id:currenId}, function(data){
				// 判断返回来的值
				if(data == 1){
					//走到这里就删除一整行
		...
		$('table').on('click', '.btn-warning', function(){
			...
			$("table").on("click", '.btn-info', function(){
				...
				//请求ajax
				$.post("<?php echo U('Friendlinks/updatalink');?>",{"id":currenId,"name":val[0],"url":val[1]},function(data){
							//判断返回值
							//
							// console.log(data);
							if ( data == 1 ) {
								// 修改成功后替换
								$('<button type="submit" class="btn btn-warning" data-value="<?php echo ($sh["id"]); ?>" >更改</button>').replaceAll(".btn-info");
