...
<div class="right" id="li07">
	<div class="right01">
		<img src="/MI-4-20/Public/Sun/img/04.gif" /> 站点管理 &gt; <span>广告列表</span>
	</div>
	<div class="rightmain">
	<!-- 加载css样式 -->
	...
	<!-- 加载js样式 -->
		...
		<center>
			...
			<div class="Boa_addboard">
				<div class="Boa_leftbox">
					<!--搜索部分开始-->
					<form action="<?php echo U('Board/boardList');?>" method="post">
						<div>
							<div class="col-xs-4">
				...
				<div class="Boa_rigthbox">
					<a href="<?php echo U('Board/addboard');?>" class="btn btn-info">添加广告</a>
				</div>	
			</div>
			<br/>
			<table class="table table-hover" >
				...
				<?php if(is_array($boardlist)): foreach($boardlist as $key=>$vo): ?><tr class="text-center">
						<td style="vertical-align: middle"><?php echo ($vo["id"]); ?></td>
						<td style="vertical-align: middle"><?php echo ($vo["name"]); ?></td>
						<?php if($vo["type"] == 0): ?><td class="type" style="vertical-align: middle">logo</td>
						...
						<?php elseif($vo["type"] == 5): ?>
							<td class="type" style="vertical-align: middle">周边</td><?php endif; ?>

						<td style="vertical-align: middle">
							<!-- for循环 -->
							<?php $__FOR_START_2096482644__=0;$__FOR_END_2096482644__=count($vo[url]);for($i=$__FOR_START_2096482644__;$i < $__FOR_END_2096482644__;$i+=1){ ?><!-- <img alt='345' height='80' width='80' src='/MI-4-20/Public/<?php echo ($vo[url][$i]); ?>' /> -->
								<img alt='345' height='80' width='80' src='/MI-4-20/Public/<?php echo ($vo[url][$i]); ?>' /><?php } ?>
						</td>

						<td style="vertical-align: middle"><?php echo ($vo["link"]); ?></td>

						<?php if($vo["status"] == 0): ?><td style="vertical-align: middle" class="status Boa_cursor" data-status="<?php echo ($vo["status"]); ?>">
							<font class="text-danger">禁用</font>	
							</td>
						<?php else: ?>
							<td style="vertical-align: middle" class="status Boa_cursor" data-status="<?php echo ($vo["status"]); ?>">
							<font class="text-success">启用</font>	
							</td><?php endif; ?>
						
						<td style="vertical-align: middle">
							<a class="btn btn-primary btn-info" href="<?php echo U('Board/updataBoard', 'id='.$vo['id']);?>">编辑</a>
		...
		<script>
			...
			$('table .del').click(function (){
				...
				$.get(
					'<?php echo U("Board/ajaxDeleteBoard");?>',//URL
			...
			for(i = 1 ; i<objs.length; i++) {
				...
				$(objs[i]).children('td').eq(5).on('click', function () {
					...
					//ajax请求
					$.get(
						'<?php echo U("Board/changeStat");?>',//URL
						{
			...
			// 	//给隐藏域里面的
			// 	$('#id').val(id);
			// 	$('#form').attr("action","<?php echo U('Board/updataBoard');?>");
