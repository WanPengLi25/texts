...
<div class="right" id="li07">
	...
	<div class="rightmain">
		<div class="ind_main center-block">
			...
			<form style="width:70%;margin:10px auto" action="<?php echo U('Type/selectType');?>" class="form-inline">
	  			<div class="form-group">
	    			<input name="id" type="text" class="form-control" id="exampleInputEmail3" placeholder="序号" value="<?php echo ($id); ?>">
	  			</div>
	  			<div class="form-group">
	    			<input name="typename" type="text" class="form-control" id="exampleInputPassword3" placeholder="分类名" value="<?php echo ($typename); ?>">
	  			</div>
	  			<select name="status" value="" class="form-control">
	  				<option value=" ">--请选择--</option>
	  			<?php if($status == 1): ?><option selected value="1">启用</option>
	  			<?php else: ?>
	  				<option value="1">启用</option><?php endif; ?>
	  			<?php if($status == 0): ?><option selected value="0">禁用</option>
	  			<?php else: ?>
			...
			<table style="width:100%;margin:0 auto;text-align:center" class="table table-hover">
				...
				<?php if($type != 0): if(is_array($type)): foreach($type as $key=>$v): ?><tr>
							<td><?php echo ($v["id"]); ?></td>
							...
							<td>
								<button type="button" data-id="<?php echo ($v["id"]); ?>" data-brands="<?php echo ($v["typename"]); ?>" class="bj btn btn-primary" data-toggle="modal" data-target="#details">编辑</button>
								
								<a  class="delete btn btn-danger"  href="javascript:;"  data-id="<?php echo ($v["id"]); ?>">删除</a>
							</td>
						</tr><?php endforeach; endif; ?>
				<?php else: ?>
					<tr><td colspan="4" style="font-size:20px" class="text-danger text-center">没有数据TT</td></tr><?php endif; ?>
			</table>
			<div  class="manu">
				<?php echo ($show); ?>
			</div>	
...
<div id="add" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
  	<div class="modal-dialog modal-lg" role="document">
    	<div class="modal-content">
    		<center>
    		<h2>添加分类</h2>
	      		<form action="<?php echo U('Type/addType');?>" method="post">
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<center>
				...
				<form action="<?php echo U('Type/saveType');?>" method="post">
...
<script>
	...
	$('.delete').click( function() {
		var that = $(this);
		$.get(
			'<?php echo U("Type/ajaxDeleteType");?>',
	...
	$('.status').click( function() {
		var that = $(this);
		$.get(
			'<?php echo U("Type/ajaxStatusType");?>',
			{
				id:that.attr('data-id'),
