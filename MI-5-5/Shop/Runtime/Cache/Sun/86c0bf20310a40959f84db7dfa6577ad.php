...
<div class="right" id="li07">
	<div class="right01">
		<img src="/MI-4-20/Public/Sun/img/04.gif" /> 站点管理 &gt; <span>轮播图</span>
	</div>
	<div class="rightmain">
	<!-- 加载css样式 -->
	...
	<!-- 加载js样式 -->
		...
		<center>
			...
			<div class="Boa_addboard">
				<div class="Boa_leftbox">
					<!--搜索部分开始-->
					<form action="<?php echo U('Shuffing/shuffingList');?>" method="post">
						<div>
							<div class="col-xs-3">
				...
				<div class="Boa_rigthbox">
					<a href="<?php echo U('Shuffing/addShuffing');?>" class="btn btn-info">添加轮播图</a>
				</div>	
			</div>
			<br/>
			<table class="table table-hover">
				...
				<?php if(is_array($shuffinglist)): foreach($shuffinglist as $key=>$vo): ?><tr class="text-center">
						<td><?php echo ($vo["id"]); ?></td>
						<td><?php echo ($vo["name"]); ?></td>
						<td>
							<!-- for循环 -->
							<?php $__FOR_START_859428595__=0;$__FOR_END_859428595__=count($vo[url]);for($i=$__FOR_START_859428595__;$i < $__FOR_END_859428595__;$i+=1){ ?><img alt='345' height='80' width='80' src='/MI-4-20/Public/<?php echo ($vo[url][$i]); ?>' /><?php } ?>
						</td>
						<td><?php echo ($vo["link"]); ?></td>
						<?php if($vo["status"] == 0): ?><td style="cursor: pointer" class="status" data-status="<?php echo ($vo["status"]); ?>"><font class="text-danger">禁用</font></td>
						<?php else: ?>
							<td style="cursor: pointer" class="status" data-status="<?php echo ($vo["status"]); ?>"><font class="text-success">启用</font></td><?php endif; ?>
						
						<td>
							<a class="btn btn-primary btn-info" href="<?php echo U('Shuffing/updataShuffing', 'id='.$vo['id']);?>">编辑</a>
		...
		<script>
			...
			$('table .del').click(function (){
				...
				$.get(
					'<?php echo U("Shuffing/ajaxDeleteShuffing");?>',//URL
			...
			for(i = 1 ; i<objs.length; i++) {
				...
				$(objs[i]).children('td').eq(4).on('click', function () {
					...
					//ajax请求
					$.get(
						'<?php echo U("Shuffing/changeStat");?>',//URL
						{
